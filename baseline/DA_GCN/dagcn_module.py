...
class DA_GCN( ):
    def __init__(self, n_items_A, n_items_B, n_users, A_in, all_h_list, all_t_list, all_v_list, embedding_size=100, n_member=2, n_layers=1,
                 n_fold=100, layer_size='[100]', regularizer_rate=0.01, gpu='0'
                 ):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                self.uid, self.seq_A, self.seq_B, self.target_A, self.target_B, self.learning_rate, self.dropout_rate, self.keep_prob = self.get_inputs()

            with tf.name_scope('encoder'):
                self.all_weights = self._init_weights()
                self.i_embeddings_A, self.u_embeddings, self.i_embeddings_B = self._create_ngcf_embed(self.n_items_A, self.n_users, self.n_items_B, self.A_in, self.n_member, self.embedding_size)
                self.seq_emb_A_output, self.seq_emb_B_output = self.encoder(self.uid, self.seq_A, self.seq_B, self.dropout_rate,self.i_embeddings_A, self.u_embeddings, self.i_embeddings_B)
    ...
    def _init_weights(self):
        ...
        all_weights['user_embedding'] = tf.Variable(initializer([self.n_users, self.embedding_size*self.n_member]), name='user_embedding')
        all_weights['item_embedding_A'] = tf.Variable(initializer([self.n_items_A, self.embedding_size*self.n_member]),
                                            name='item_embedding_A')
        all_weights['item_embedding_B'] = tf.Variable(initializer([self.n_items_B, self.embedding_size*self.n_member]),
                                            name='item_embedding_B')
        self.layers_plus = [self.embedding_size] + self.weight_size
        for k in range(self.n_layers):
            all_weights['W_gc_%d' % k] = tf.get_variable(
                    'W_gc_%d' % k, [self.layers_plus[k]*self.n_member, self.layers_plus[k + 1]*self.n_member], tf.float32, initializer)
            ...
            all_weights['W_bi_%d' % k] = tf.get_variable(
                    'W_bi_%d' % k, [self.layers_plus[k]*self.n_member, self.layers_plus[k + 1]*self.n_member], tf.float32, initializer)
    ...
    def _generate_transE_score(self):
        embeddings = tf.concat([self.all_weights['item_embedding_A'], self.all_weights['user_embedding'], self.all_weights['item_embedding_B']], axis=0)
        ...
        #kg_score = tf.reduce_sum(tf.multiply(h_e, t_e), 1)
    ...
    def _create_ngcf_embed(self, n_items_A, n_users, n_items_B,A_in, n_member,embedding_size):
        ...
        ego_embeddings = tf.concat([self.all_weights['item_embedding_A'], self.all_weights['user_embedding'], self.all_weights['item_embedding_B']], axis=0)
        ...
        for i in range(self.n_layers+1):
            i_g_embeddings_A, u_g_embeddings_i, i_g_embeddings_B = tf.split(all_embeddings[i], [n_items_A, n_users, n_items_B],
                                                                      0)
    ...
    def prediction_A(self, n_items_A, seq_emb_B_output, seq_emb_A_output, keep_prob):
        with tf.variable_scope('prediction_A'):
            ...
            #concat_output = seq_emb_A_output
            print(concat_output)
    ...
    def prediction_B(self, n_items_B, seq_emb_A_output, seq_emb_B_output, keep_prob):
        ...
        with tf.variable_scope('prediction_B'):
            ...
            #concat_output = seq_emb_B_output
            print(concat_output)
            concat_output = tf.nn.dropout(concat_output,
                                          keep_prob)
